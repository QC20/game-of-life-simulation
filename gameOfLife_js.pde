...
class Cell {
  ...
  update() {
    ...
    //add turbulence
    //noise function seems to not be perfectly in range of [0.0-1.0],
...
function draw() {
  ...
  //console.log(cells.length + " / " + m_maxCells + " fps: " + frameRate() + " | " + m_spawnProb);
  ...
  updateCellVelocities(); // Call the function to update cell velocities based on arrow key controls
}

function keyPressed() {
  // Set arrow key variables to true when pressed
  if (keyCode === UP_ARROW) {
    upKeyPressed = true;
  ...
  } else if (key === 'c' || key === 'C') {  // Check if the 'c' key is pressed
    // Cycle through the colors
    colorIndex = (colorIndex + 1) % 10;
    // Set the new color for all cells
    for (let i = 0; i < cells.length; i++) {
      let c = cells[i];
...
function keyReleased() {
  // Set arrow key variables to false when released
  if (keyCode === UP_ARROW) {
    upKeyPressed = false;
  } else if (keyCode === DOWN_ARROW) {
...
// Update cell velocities based on arrow key controls
...
// Define a function to get the color based on the index
